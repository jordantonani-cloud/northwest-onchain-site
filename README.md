...
Brand tokens are first-class Tailwind utilities ([`tailwind.config.mjs`](tailwind.config.mjs)) mirrored as CSS custom properties in [`src/styles/global.css`](src/styles/global.css). The "Topographic Mesh" motif (PNW contour lines as a blockchain node-network) lives in `TopoLines`, `PeakMark`, and `NodeGlow`.

WCAG 2.2 AA verified (Lighthouse a11y 100): skip link, visible focus, keyboard-operable nav/FAQ/forms, semantic landmarks, `prefers-reduced-motion` disables motion, AA contrast throughout. One intentional change from the prototype: the light-section accent was darkened from `#1F8D72` (3.6:1 — fails) to **`#0E7458` (5.1:1)**.
...
Brand, copy, and pricing follow `BRAND-BRIEF.md` and the approved prototype. Generated by milestone; see the git history for the build sequence.
